...
public @interface S2Action {
    ...
    /**
     * アクションが実行可能であるための条件を式言語で記述します。式はアクションオブジェクトを起点として評価され、
     * {@code true} を返すときに限りアクションが実行可能となります。この引数が指定されていない場合は、アクションは常に実行可能となります。
     */
    String enabled() default "";

    /**
     * アクションが選択状態となるための条件を式言語で記述します。式はアクションオブジェクトを起点として評価され、
     * {@code true} を返すときに限りアクションが選択状態となります。この引数が指定されていない場合は、アクションは常に非選択状態となります。
     */
