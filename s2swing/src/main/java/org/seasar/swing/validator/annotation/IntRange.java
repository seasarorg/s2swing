...
import org.seasar.swing.validator.IntRangeConstraint;

/**
 * {@code int}型の数値範囲を指定する制約アノテーションです。
 * <p>
 * このアノテーションに対応する、リソースファイル中でのカスタムエラーメッセージのキーとその内容は、以下の通りです。
 * 
 * <ul>
 * <li>{@code [propertyName].IntRange.min.failed} -
 * 最小値に違反した場合のメッセージ。変数として{min}を使用可能</li>
 * <li>{@code [propertyName].IntRange.max.failed} -
 * 最大値に違反した場合のメッセージ。変数として{max}を使用可能</li>
 * <li>{@code [propertyName].IntRange.both.failed} -
